...
def put_data(uri, param=None, vmanage_ip=vmanage_ip):
    if not param:
        raise Exception("No data to post")
    session, token = authenticate(vmanage_ip, username, password)
    url = f"https://{vmanage_ip}{uri}"
    headers = {"X-XSRF-TOKEN": token}
    response = session.put(url, headers=headers, json=param, verify=False)
    if response.status_code == 200:
        return response
    else:
        print(response, response.text)
...
def get_alarms():
    query =  {  "query": {    "condition": "AND",    "rules": [      {        "field": "entry_time",        "operator": "last_n_days",        "type": "date",        "value": [          "500"        ]      },      {        "field": "active",        "type": "boolean",        "value": [          "true"        ],        "operator": "equal"      }    ]  }}
    result = get_data("/dataservice/alarms", query2=query)["data"]
    #pprint(result)
