...
class AlarmAnalyzer:
    ...
    def _prepare_data(self):
        ...
        for alarm in self.raw_data:
            ...
            # Map dnac_issues fields to expected fields
            # type: use category if present, else status, else name
            flat_alarm['type'] = flat_alarm.get('category') or flat_alarm.get('status') or flat_alarm.get('name')
            # entry_time: use last_occurence_time (convert ms to datetime)
            if 'last_occurence_time' in flat_alarm:
                flat_alarm['entry_time'] = pd.to_datetime(flat_alarm['last_occurence_time'], unit='ms')
            else:
                flat_alarm['entry_time'] = pd.NaT
            # severity: use priority if present, else status, else 'Unknown'
            flat_alarm['severity'] = flat_alarm.get('priority') or flat_alarm.get('status') or 'Unknown'
            # component: use deviceId if present, else name
            flat_alarm['component'] = flat_alarm.get('deviceId') or flat_alarm.get('name')
            # uuid: use issueId
            flat_alarm['uuid'] = flat_alarm.get('issueId')
            # system_ip: use deviceId (for compatibility)
            flat_alarm['system_ip'] = flat_alarm.get('deviceId')
            # message: use name
            flat_alarm['message'] = flat_alarm.get('name')
            flattened_data.append(flat_alarm)
        df = pd.DataFrame(flattened_data)
        # Only convert entry_time, as others are not present
...
def analyze_alarms(alarms_file='dnac_issues.json', use_clustering=True, eps=0.1, min_samples=2):
    """
    ...
    output_file = 'alarm_analysis_results_dnac.json'
...
if __name__ == "__main__":
    results = analyze_alarms('dnac_issues.json')
