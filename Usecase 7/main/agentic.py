...
def agentic():
    ...
    for alarm in alarmlist["Alarms"]:
        ...
        if alarm["Status"].lower() == "resolved":
            ...
            #TODO: In future to derive the traffic path from the LLM by asking it to analyze the alarm and topology : 
            response_traffic_path = dochat.dochat(messages=messages,json=True)
            if type(response_traffic_path["traffic-path"]) == str:
                ...
                response_traffic_path["traffic-path"] = eval(response_traffic_path["traffic-path"])
            
            #response_traffic_path = {"traffic-path": ["branch-router", "branch-fw", "server", "remote-worker"]}
